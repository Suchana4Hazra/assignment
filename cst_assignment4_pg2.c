...
int dequeue(queue *q,int *qElement,int *rValue)
{
	if(q->front==-1)
	{
		printf("\n queue is empty");
	...
	*rValue=rand()%10;
	...
	if(isQueueFull(q)==1)
	{
		printf("\n back enqeue not possible.");
		return 0;
	}
	else
	{
		enqueueInteger(q,*qElement-*rValue);
	}
	return 1;
...
main()
{
	...
	do{
		...
		switch(ch)
		{
		    ...
		    case 2:
		    	n=dequeue(&queue1,&dig,&ranVal);
		    	if(n==1)
		    	{
		    	printf("\n popped element: %d",dig);
		    	printf("\n random value: %d",ranVal);
